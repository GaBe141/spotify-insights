...
class AdvancedStreamingAnalytics:
    ...
    def run_quality_analysis(self) -> Dict[str, Any]:
        ...
        for source, df in self.processed_data.items():
            ...
            value_cols = [col for col in df.columns if col != timestamp_col and df[col].dtype in ['int64', 'float64']]
        ...
        for source, df in self.processed_data.items():
            ...
            for col in value_cols[:3]:  # Limit to first 3 columns for performance
                try:
                    ...
                    # Print summary
                    model_count = len([m for m in col_results['models'].values() 
                                     if isinstance(m, dict) and m.get('success', False)])
                    forecast_count = len([f for f in col_results['forecasts'].values() 
                                        if 'forecast' in f])
    ...
    def generate_insights(self) -> Dict[str, Any]:
        ...
        high_priority = [rec for rec in all_recommendations if any(word in rec.lower() 
                        for word in ['high', 'critical', 'urgent', 'error'])]
        medium_priority = [rec for rec in all_recommendations if any(word in rec.lower() 
                          for word in ['moderate', 'warning', 'consider'])]
        low_priority = [rec for rec in all_recommendations if rec not in high_priority + medium_priority]
    ...
    def create_comprehensive_report(self, output_dir: str = "data/advanced_analysis") -> str:
        ...
        try:
            viz_path = visualize_comprehensive_results(str(results_file), str(output_path / "visualizations"))
