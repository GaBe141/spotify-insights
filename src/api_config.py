...
class SocialAPIManager:
        ...
        if self.config_file.exists():
            try:
                ...
                for platform, config_data in data.items():
                    self.configs[platform] = APIConfig(
                        platform=platform,
                        **config_data
                    )
    ...
    def get_config(self, platform: str) -> Optional[APIConfig]:
        """Get configuration for a platform."""
        return self.configs.get(platform.lower())
    
    def set_api_key(self, platform: str, api_key: str, secret_key: str = "", 
    ...
    def can_make_request(self, platform: str) -> bool:
        ...
        if config.requests_per_minute > 0 and config.requests_this_minute >= config.requests_per_minute:
            return False
...
if __name__ == "__main__":
    ...
    for platform, details in status["platforms"].items():
        ...
        print(f"   {status_icon} {platform.title()}: {cred_icon} "
