...
def plot_artist_popularity_vs_rank(time_ranges=['short_term', 'medium_term', 'long_term']) -> Path:
    ...
    for i, (time_range, color, label) in enumerate(zip(time_ranges, colors, labels)):
...
def plot_top_artist_genres() -> Path:
    ...
    wedges, texts, autotexts = ax1.pie(top_genres.values, labels=top_genres.index, 
    ...
    pivot_data = genre_time_filtered.pivot(index='time_range', columns='genre', values='count').fillna(0)
...
def plot_track_length_distribution() -> Path:
    ...
    if all_durations:
        ...
        axes[3].axvline(mean_duration, color='red', linestyle='--', 
...
def plot_discovery_timeline() -> Path:
    ...
    ax1.plot(daily_stats.index, daily_stats['unique_tracks'], 
             marker='o', linewidth=2, color='#1DB954', markersize=6)
    ...
    ax2.plot(daily_stats.index, daily_stats['unique_artists'], 
             marker='s', linewidth=2, color='#FF6B6B', markersize=6)
