...
class EnhancedViralPredictor:
    ...
    def predict_peak_timing(
        self, momentum: float, acceleration: float, current_score: float
    ...
    def assess_risk_level(self, viral_score: float, confidence: float, momentum: float) -> str:
        """
    ...
    def predict_viral_potential(self, track_data: dict[str, Any]) -> ViralMetrics:
        ...
        viral_score = platform_avg * 0.40 + social_score * 0.35 + audio_score * 0.25
        ...
        data_confidence = min(
            1.0, len(historical_data) / 10
        )  # More historical data = higher confidence

        score_consistency = 1.0 - (
            np.std(list(platform_scores.values())) / 100 if platform_scores else 0.5
        )

        confidence = platform_confidence * 0.4 + data_confidence * 0.3 + score_consistency * 0.3
    ...
    ) -> list[tuple[dict[str, Any], ViralMetrics]]:
        """
    ...
    def print_prediction(self, track_name: str, metrics: ViralMetrics):
        ...
        print(
            f"Peak ETA:           {metrics.peak_eta_days:>3} days (range: {metrics.peak_confidence_interval[0]}-{metrics.peak_confidence_interval[1]} days)"
        )
...
def demo():
    ...
    sample_track = {
        ...
        "platform_scores": {"spotify": 85, "tiktok": 92, "youtube": 78, "instagram": 88},
    ...
    for i, (track, metrics) in enumerate(results, 1):
        print(f"\n#{i}. {track['track_name']}")
        print(
            f"    Viral Score: {metrics.viral_score:.1f} | "
