...
class TemporalAnalyzer:
    ...
    def __init__(self, data_dir: str = "data"):
        ...
        self.listening_history: Optional[pd.DataFrame] = None
    ...
    def _generate_sample_data(self) -> pd.DataFrame:
        ...
        tracks = [
            "Vampire", "Flowers", "Anti-Hero", "Calm Down", "As It Was",
            "Unholy", "Heat Waves", "Levitating", "Blinding Lights", "drivers license"
        ] * (num_records // 10)
        
        artists = [
            "Olivia Rodrigo", "Miley Cyrus", "Taylor Swift", "Rema", "Harry Styles",
            "Sam Smith", "Glass Animals", "Dua Lipa", "The Weeknd", "Olivia Rodrigo"
        ] * (num_records // 10)
    ...
    def analyze_weekly_patterns(self) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            'most_active_day': daily_stats['play_count'].idxmax() if not daily_stats.empty else None
        }
    ...
    def analyze_genre_time_patterns(self) -> Dict[str, Any]:
        ...
        for time_cat in artist_time['time_category'].unique():
            ...
            top_artists = time_data.nlargest(5, 'count')[['artist_name', 'count']].to_dict('records')
    ...
    def generate_comprehensive_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            'data_range': {
                'start': self.listening_history['played_at'].min().isoformat() if not self.listening_history.empty else None,
                'end': self.listening_history['played_at'].max().isoformat() if not self.listening_history.empty else None,
                'total_records': len(self.listening_history) if self.listening_history is not None else 0
            },
    ...
    def print_insights(self, report: Optional[Dict[str, Any]] = None):
        ...
        if report['hourly_patterns']:
            ...
            print("\n🕐 Peak Listening Hours:")
            print(f"   Most active: {hourly['peak_listening_hour']}:00 ({hourly['peak_hour_plays']} plays)")
            print(f"   Most active period: {hourly['most_active_period'].replace('_', ' ').title()}")
            
            print("\n   Time Period Distribution:")
            for period, count in sorted(hourly['period_distribution'].items(), 
                                       key=lambda x: x[1], reverse=True):
                percentage = (count / report['data_range']['total_records']) * 100
                bar = "█" * int(percentage / 2)
                print(f"   {period.replace('_', ' ').title():<15} {bar:<25} {count:>3} plays ({percentage:>5.1f}%)")
        ...
        if report['weekly_patterns']:
            ...
            print(f"   Weekday plays: {weekly['weekday_plays']} (avg {weekly['weekday_avg']:.1f}/day)")
            print(f"   Weekend plays: {weekly['weekend_plays']} (avg {weekly['weekend_avg']:.1f}/day)")
            ...
            for time_cat, artists in report['genre_time_patterns']['by_time_of_day'].items():
                if artists:
                    ...
                    for artist_data in artists[:3]:
                        print(f"      • {artist_data['artist_name']} ({artist_data['count']} plays)")
