...
def simple_multi_source_demo():
        ...
        for _, artist_row in top_artists.head(3).iterrows():  # Just top 3 for demo
            ...
            if audiodb_details and audiodb_details['name']:
                audiodb_enriched.append({
                    ...
                    'audiodb_biography': audiodb_details['biography'][:200] + "..." if audiodb_details['biography'] else None
                })
            else:
                audiodb_enriched.append({
                    'spotify_name': artist_name,
    ...
    if not audiodb_df.empty:
        ...
        insights['audiodb_data'] = {
            ...
            'average_formation_year': int(pd.to_numeric(formed_years, errors='coerce').mean()) if len(formed_years) > 0 else None
        }
    ...
    print(f"🎵 Spotify: {insights['spotify_data']['total_artists']} artists, {insights['spotify_data']['total_tracks']} tracks")
    ...
    if 'musicbrainz_data' in insights:
        ...
        if mb_data['unique_countries'] > 0:
            print(f"   Countries: {mb_data['unique_countries']} ({mb_data['top_country']} most common)")
