...
def test_data_quality_analysis():
    ...
    if "outlier_analysis" in quality_report:
        ...
        for col, outliers in quality_report["outlier_analysis"].items():
            if "iqr" in outliers:
                print(
                    f"   {col}: {outliers['iqr']['count']} outliers ({outliers['iqr']['percentage']:.1f}%)"
                )
...
def test_forecasting_engine():
    ...
    for model_name, forecast in forecasts.items():
        if isinstance(forecast, dict) and "error" not in forecast and "forecast" in forecast:
            ...
            print(
                f"   ✅ {model_name}: {len(forecast_values)} forecasts, mean={np.mean(forecast_values):.1f}"
            )
        else:
            error_msg = (
                forecast
                if isinstance(forecast, str)
                    ...
                    forecast.get("error", "No forecast generated")
                    if isinstance(forecast, dict)
                    else "No forecast generated"
                )
            )
    ...
    for col, col_results in results["forecasting"].items():
        if "error" not in col_results:
            model_count = len(
                [
                    m
                    for m in col_results.get("models", {}).values()
                    if "success" in m and m["success"]
                ]
            )
            forecast_count = len(
                [f for f in col_results.get("forecasts", {}).values() if "forecast" in f]
            )
...
def demonstrate_statistical_features():
    ...
    viral_peak = np.array([800 + i * 200 for i in range(10)] + [2800 - i * 150 for i in range(10)])
    ...
    viral_data = pd.DataFrame(
        {"date": dates, "streams": streams, "plays": streams * 0.85, "listeners": streams * 0.4}
    )
