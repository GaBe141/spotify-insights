...
def test_basic_functionality():
    ...
    data = pd.DataFrame(
        {"date": dates, "streams": streams, "plays": streams * 0.9, "listeners": streams * 0.4}
    )
    ...
    try:
        quality_report = analyzer.analyze_data_quality(
            data=data, timestamp_col="date", value_cols=["streams", "plays", "listeners"]
        )
        ...
        if "temporal_analysis" in quality_report:
            ...
            if "date_range" in temporal:
                print(
                    f"      Date range: {temporal['date_range']['start']} to {temporal['date_range']['end']}"
                )
...
def test_data_preparation():
    ...
    data = pd.DataFrame({"date": dates, "streams": streams})
