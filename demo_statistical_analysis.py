...
def analyze_spotify_track_data(df):
    ...
    numeric_cols = []
    potential_numeric = ['popularity', 'duration_ms', 'danceability', 'energy', 
...
def analyze_listening_history(df):
    ...
    try:
        ...
        if 'daily_plays' in outlier_analysis and 'iqr' in outlier_analysis['daily_plays']:
            ...
            if outlier_pct > 10:
                print(f"      🚨 UNUSUAL LISTENING DETECTED: {outlier_pct:.1f}% of days show abnormal activity")
            elif outlier_pct > 5:
    ...
    for filename, df in data_files.items():
        ...
        if 'track' in filename.lower():
            ...
            if result and len(result) == 2:
                ...
                if avg_variety > 0.8:
                    insights['listening_insights'].append("High music discovery rate - explores many artists")
                elif avg_variety > 0.5:
                    insights['listening_insights'].append("Moderate music discovery rate")
                else:
                    insights['listening_insights'].append("Focused listening - tends to repeat favorite artists")
