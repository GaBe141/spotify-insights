...
class AudoraMusicExplorer:
    ...
    def generate_insights(self):
        ...
        # Session insights
        insights.append(f"🔍 Tracks analyzed this session: {self.current_session['analyzed_tracks']}")
    ...
    def search_collection(self, query):
        ...
        if unique_results:
            ...
            for i, result in enumerate(unique_results[:10], 1):
                print(f"{i:2}. {result['track'][:35]:<35} - {result['artist'][:25]:<25} ({result['dataset']})")
    ...
    def show_viral_candidates(self):
        ...
        if high_potential:
            ...
            for i, candidate in enumerate(high_potential, 1):
                ...
                print(f"   Viral Score: {candidate['viral_score']:.1f}  Confidence: {candidate['confidence']:.1%}")
        else:
        ...
        while True:
            try:
                ...
                if cmd == 'exit' or cmd == 'quit':
                    ...
                    session_time = datetime.now() - self.current_session['start_time']
                    print(f"   • Time spent: {session_time.seconds // 60}m {session_time.seconds % 60}s")
                    break
