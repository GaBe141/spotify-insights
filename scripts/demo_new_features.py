...
from mood_playlist_generator import MoodPlaylistGenerator, MoodCategory  # noqa: E402
...
class AudoraFeatureShowcase:
    ...
    def demo_temporal_analysis(self):
        ...
        if report['listening_streaks']:
            ...
            if streaks['longest_streak'] > 1:
                print(f"   • Longest listening streak: {streaks['longest_streak']} consecutive days! 🔥")
    ...
    def demo_enhanced_viral_prediction(self):
        ...
        for i, (track, metrics) in enumerate(results, 1):
            ...
            print(f"   Peak in:             {metrics.peak_eta_days:>3} days (±{(metrics.peak_confidence_interval[1] - metrics.peak_confidence_interval[0])//2} days)")
        ...
        if temporal_report['data_range']['total_records'] > 0:
            ...
            if temporal_report['weekly_patterns']:
                ...
                if weekday_avg > weekend_avg:
                    print(f"   • Listening style: More active on weekdays ({weekday_avg:.1f} vs {weekend_avg:.1f} plays/day)")
                else:
                    print(f"   • Listening style: Weekend music enthusiast ({weekend_avg:.1f} vs {weekday_avg:.1f} plays/day)")
        
        # Mood-Time correlations
        print("\n🌟 Mood-Time Insights:")
        if temporal_report.get('genre_time_patterns') and temporal_report['genre_time_patterns'].get('by_time_of_day'):
